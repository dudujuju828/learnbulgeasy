  ...
  try {
    ...
    for (const heap of toInsert) {
      await sql`
        ...
        VALUES (${heap.name}, ${heap.description}, ${heap.theme}, ${heap.order}, ${JSON.stringify(heap.words)}::jsonb, 2)
      `
    }

    return NextResponse.json({ success: true, message: `Seeded ${toInsert.length} Map 2 heaps`, inserted: toInsert.length })
  } catch (err) {
